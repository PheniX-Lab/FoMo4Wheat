...
from dinov2.utils.cluster import (
    ...
    get_slurm_executor_parameters_load_cp
)
...
def submit_jobs(task_class, args, name: str):
    ...
    #executor.update_parameters(slurm_partition='gpu8',slurm_reservation='2023222003_2')
    executor.update_parameters(slurm_partition='gpu8')
...
def submit_load_cp_jobs(task_class, args, name: str):
    if not args.output_dir:
        args.output_dir = str(get_shared_folder() / "%j")
    ...
    executor_params = get_slurm_executor_parameters_load_cp(
        nodes=args.nodes,
        num_gpus_per_node=args.ngpus,
        timeout_min=args.timeout,  # max is 60 * 72
        slurm_signal_delay_s=120,
